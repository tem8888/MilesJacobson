module.exports = {
	...
	execute(message, args) {
		...
		if (message.channel.type === 'dm') {
			...
			let registrationChannelID = '740148769391247451'
			let regMsg = message.content.toString();
      let regMsgLen = message.content.split("\n").length-1;

      if (regMsgLen >= 5) {
          message.client.channels.cache.get(registrationChannelID).send(`>>> Новая регистрация от пользователя **${message.author}**\n${message.content.slice(7)}`);
